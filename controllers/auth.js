...
exports.getTutors = async (req, res, next) => {
    try {
        const tutorsList = await User.find({role: "tutor"}, '_id role username email name');
...
exports.getTutor = async (req, res, next) => {
    ...
    try {
        ...
        if(user.role !== 'tutor') return res.status(400).json({
            status: false,
            message: "This user is not a tutor"
        });
        try {
            const user = await User.findById({_id: tutorId}, '_id role username email name');
...
exports.deleteTutor = async(req, res, next) => {
    ...
    try {
        ...
        try {
            await User.deleteOne({_id: tutorId});
            res.status(200).json({
                status: true,
            ...
            res.status(500).json({
                status: false,
                message: "Unable to delete the tutor"
            })
        }
    } catch(err) {
        res.status(400).json({
